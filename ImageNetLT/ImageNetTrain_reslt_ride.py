...
model_names += ['resnext50_32x4d_reslt','resnet10_reslt', 'resnext101_32x4d_reslt', 'ResNeXt50Model']
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        loss, logit = 0, 0
        for k in range(args.num_experts):
            loss += ResLTLoss(logits_list[k], (labelH, labelM, labelT))
            logit += logits_list[k][0] + logits_list[k][1] + logits_list[k][2]
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            for i in range(args.num_experts):
                output += logits_list[i][0] + logits_list[i][1] + logits_list[i][2]
            output = output / args.num_experts 
